...
class GRPOArguments(GRPOConfigTRL):
    ...
    lambda_sem: float = field(default=0.01, metadata={"help": "Weight for semantic latent consistency reward"})
    lambda_vis: float = field(default=0.01, metadata={"help": "Weight for visual latent consistency reward"})
    tau_sem: float = field(default=1.0, metadata={"help": "Tolerance margin for semantic latent stability"})
    tau_vis: float = field(default=1.0, metadata={"help": "Tolerance margin for visual latent stability"})
    lambda_consistency: float = field(default=0.05, metadata={"help": "Overall weight for consistency reward"})
    lambda_stability: float = field(default=0.05, metadata={"help": "Overall weight for stability reward"})
...
class DataArguments:
    ...
    dataset_sample_ratio: float = field(
        default=1.0,
        metadata={"help": "Fraction of dataset to use for training (0 < ratio <= 1.0). Set to 1.0 to use full dataset."}
    )
