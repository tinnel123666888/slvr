...
from src.model.slvr_heads import SLVRHead, SLVRHeadGLU
...
class QwenWithSLVR(Qwen2_5_VLForConditionalGeneration):
    ...
    def generate(
        ...
        **kwargs,
        ...
        # transformers 不同版本对 assistant 校验接口不一致；不存在时跳过以保持兼容
        ...
        # transformers 不同版本 _prepare_cache_for_generation 签名不同：新版已移除 device 参数
        _cache_params = set(inspect.signature(self._prepare_cache_for_generation).parameters.keys())
        ...
        # transformers 不同版本 _get_stopping_criteria 签名不同；按实际签名只传兼容参数
        _stopping_params = set(inspect.signature(self._get_stopping_criteria).parameters.keys())
        _stopping_kwargs = {k: v for k, v in {"generation_config": generation_config, "stopping_criteria": stopping_criteria, "tokenizer": tokenizer, **kwargs}.items() if k in _stopping_params}
        prepared_stopping_criteria = self._get_stopping_criteria(**_stopping_kwargs)
    ...
    def _slvr_deocding_with_latentend(
            ...
            **model_kwargs,
            ...
            while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
                ...
                # ---- Force <|vision_end|> during SLVR latent mode ----
                # During SLVR mode the model reasons in hidden-state space; the
                # sampled token is just a placeholder.  Forcing it to <|vision_end|>
                # keeps the KV-cache context clean so that post-SLVR text generation
                # remains coherent (matches the token pattern seen during SFT).
