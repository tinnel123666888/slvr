    ...
    def _lvr_deocding_with_latentend(
            ...
            **model_kwargs,
            ...
            while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
                ...
                # ---- Force <|vision_end|> during LVR latent mode ----
                # During LVR mode the model reasons in hidden-state space; the
                # sampled token is just a placeholder.  Forcing it to <|vision_end|>
                # keeps the KV-cache context clean so that post-LVR text generation
